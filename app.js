...
// We no longer store large default vault data as a base64 string in this file.
// The initial vault records for each campus (RIC2/RIC3) are provided via
// separate dataset scripts (vault_part1.js, vault_part2.js, vault_ric2.js).
// Each of those scripts populates `window.DEFAULT_VAULTS` by appending
// objects to the array.  When initializing state.vaults in loadState(),
// we'll fall back to window.DEFAULT_VAULTS if no saved vaults exist.
const DEFAULT_VAULTS = (typeof window !== 'undefined' && window.DEFAULT_VAULTS) ? window.DEFAULT_VAULTS : [];
